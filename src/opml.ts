...
export function importOpml(filePath: string): { imported: number; skipped: number; errors: number } {
  try {
    ...
    const opml = parser.parse(content) as OpmlStructure;
...
export function exportOpml(outputPath: string): number {
